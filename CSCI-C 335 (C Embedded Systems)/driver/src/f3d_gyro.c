...
void f3d_gyro_interface_init() {
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOA, ENABLE); //clock for A
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOE, ENABLE); //clock for E

  GPIO_StructInit(&GPIO_InitStructure);

  //SCK PA5 
  //GPIO_InitStructure.GPIO_Pin = GPIO_Pin_5;
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF;      //output, not input
...
void f3d_gyro_write(uint8_t* pBuffer, uint8_t WriteAddr, uint16_t NumByteToWrite) {
  ...
  while(NumByteToWrite > 0x00) {
    f3d_gyro_sendbyte(*pBuffer);  //added
    NumByteToWrite--;
    pBuffer++;
  }
...
static uint8_t f3d_gyro_sendbyte(uint8_t byte) {
  /*********************************************************/
  /***********************CODE HERE ************************/
  //poll transmitter status prior to writing byte
  while(SPI_I2S_GetFlagStatus(SPI1, SPI_I2S_FLAG_TXE) == RESET);
  SPI_SendData8(SPI1, byte);

  //poll to confirm we recieved full char
