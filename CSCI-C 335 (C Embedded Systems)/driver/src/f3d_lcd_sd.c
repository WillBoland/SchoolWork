...
void f3d_lcd_sd_interface_init(void) {
 /* vvvvvvvvvvv pin initialization for the LCD goes here vvvvvvvvvv*/ 
  ...
  RCC_AHBPeriphClockCmd(RCC_AHBPeriph_GPIOB, ENABLE); //port B clock
  ...
  //pins for port B, standard function
  ...
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AF; //alternate function mode, for 13-15 SPI2
  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_13; //13
  GPIO_PinAFConfig(GPIOB, 13, GPIO_AF_5); //AF5
  GPIO_Init(GPIOB, &GPIO_InitStructure);

  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14; //14
  GPIO_PinAFConfig(GPIOB, 14, GPIO_AF_5); //AF5
  GPIO_Init(GPIOB, &GPIO_InitStructure);

  GPIO_InitStructure.GPIO_Pin = GPIO_Pin_15; //15
  GPIO_PinAFConfig(GPIOB, 15, GPIO_AF_5); //AF5
  GPIO_Init(GPIOB, &GPIO_InitStructure);
  ...
  SPI_Init(SPI2, &SPI_InitStructure); //changed
  SPI_RxFIFOThresholdConfig(SPI2, SPI_RxFIFOThreshold_QF); //changed
  SPI_Cmd(SPI2, ENABLE); //changed  
} 
